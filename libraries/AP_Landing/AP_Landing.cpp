        ...
        const float height, const float sink_rate, const float wp_proportion, const uint32_t last_flying_ms, const bool is_armed, const bool is_flying, const bool rangefinder_state_in_range)
{
    ...
    default:
        ...
        gcs().send_text(MAV_SEVERITY_CRITICAL, "[02200]Landing configuration error, invalid LAND_TYPE");
        success = true;
        break;
    }
    Log();
...
bool AP_Landing::restart_landing_sequence()
{
            ...
            mission.set_current_cmd(current_index+1))
    {
        ...
        gcs().send_text(MAV_SEVERITY_NOTICE, "[02500]Restarted landing sequence. Climbing to %dm", cmd.content.location.alt/100);
        success =  true;
    }
            ...
            mission.set_current_cmd(do_land_start_index))
    {
        ...
        gcs().send_text(MAV_SEVERITY_NOTICE, "[02501]Restarted landing via DO_LAND_START: %d",do_land_start_index);
        success =  true;
    }
               ...
               mission.set_current_cmd(prev_cmd_with_wp_index))
    {
        ...
        gcs().send_text(MAV_SEVERITY_NOTICE, "[02501]Restarted landing sequence at waypoint %d", prev_cmd_with_wp_index);
        success =  true;
    } else {
        gcs().send_text(MAV_SEVERITY_WARNING, "[02400]Unable to restart landing sequence");
        success =  false;
    }
